...
class ListingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show displays interested volunteers link for org owners" do
    sign_in_as(@user_one) # owner of org one
    Interest.create!(user: @user_two, listing: @listing)
    get listing_path(@listing)
    assert_response :success
    assert_match "Interested Volunteers", response.body
  end

  test "show does not display interested volunteers link for non-owners" do
    sign_in_as(@user_two) # member but not owner
    Interest.create!(user: @user_two, listing: @listing)
    get listing_path(@listing)
    assert_response :success
  ...
  test "show does not display interested volunteers link for non-members" do
    non_member = User.create!(name: "Non Member", email_address: "nonmember@example.com", password: "password123")
    sign_in_as(non_member)
    Interest.create!(user: @user_two, listing: @listing)
    get listing_path(@listing)
